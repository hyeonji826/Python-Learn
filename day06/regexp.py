...
def search_string(target_str:str,pattern=r'\d+'):
    ...
    # 검사하고자 하는 텍스트는 전달하여 해당되는 문자열 추출
    # .findall()
...
if __name__ =="__main__":
    ...
    # r'\w+' : 하나 이상의 문자를 의미한다.
    # \w는 영어, 한글, 숫자 그리고 언더바(_) 가리킨다.
    # +는 앞에 오는 문자열이 패턴에 여러글자가 맞으면 
    # 하나로 묶어서 가져온다.
    # +를 안쓰면 하나씩 가져온다.
    ...
    results =''.join(results) # 하나로 붙여서 출력
